...
def enhancedFeatureExtractorDigit(datum):
    ...
    1: A count of the number of black pixels. It is like a very simple Area
    2: A count of black pixels whose up, down, left or right pixels are white. It is like a very simple Perimeter
    3: A very, very rudimentary edge detection by something called gradient in Image Processing.
    The values have been scaled up/down so they are almost in the same range
    ##
    """
    ...
    #"*** YOUR CODE HERE ***"
    count_1 = datum.count(1.0)

    per, red = 0,0

    for i in range(1,49): #Given the training set, we know that the outer border is "white"
        for j in range(1,49):
            p = getPixelVal(i, j)
            rt = getPixelVal(i, j+1)
            if p != rt: #change/gradient
                red += 1
            if p == 1: #black
                if rt == 0 or getPixelVal(i, j-1) == 0 or getPixelVal(i+1, j) == 0 or getPixelVal(i-1, j) == 0: #Find the 4 direction pixels and check
                    per+=1
