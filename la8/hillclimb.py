...
def generate2optNeighbours(tour):
	...
	# "*** YOUR CODE HERE ***"
	all_possible_neighbours = [tour[:i]+tour[i:j+1][::-1]+tour[j+1:] for i in range(1,cities-1) for j in range(i+1,cities) if j-i!=cities-2]
	# print(len(all_possible_neighbours)==l*(l-3)/2)
	# "*** --------------  ***"
...
def generate3optNeighbours(tour):
	...
	def four(p,q,r):
		...
		l1 = q[::-1]
		l2 = r[::-1]
		return [l0+l1+r, l0+q+l2, p+l1+l2, l0+l1+l2]

	all_possible_neighbours = [w for i in range(cities) for j in range(i+2,cities) for k in range(j+2,min(cities,(i-2)+cities+1)) for w in four(tour[i:j],tour[j:k],tour[k:]+tour[:i])]
...
def hillClimbFull(initial_tour, getNeighbours):
	...
	while True:
		temp = getNeighbours(minTour)
		tourcosts = [getTourLength(i) for i in temp]
		index = min(range(len(tourcosts)), key=tourcosts.__getitem__)
		mincost = tourcosts[index]
		if mincost > getTourLength(minTour):
			tourLengthList.append(mincost)
			break
		tourLengthList.append(mincost)
		minTour = temp[index][:]
...
def nearestNeighbourTour(initial_city):
	...
	# "*** YOUR CODE HERE ***"
	visited = set([initial_city])
	tour.append(initial_city)
	while len(visited)!=cities:
		m = min([(getDistance(nodeDict[initial_city], nodeDict[i]), i) for i in nodeDict if i not in visited])
		tour.append(m[1])
		visited.add(m[1])
		initial_city = m[-1]
	# "*** --------------  ***"
...
def eucledianTour(initial_city):
	...
	# "*** YOUR CODE HERE ***"
	# part 1
	edgeList = [[i,j,getDistance(nodeDict[i], nodeDict[j])] for i,j in itertools.combinations(nodeDict, 2)]
	# "*** --------------  ***"
...
def finalOrder(mst, initial_city):
	...
	while len(s) > 0:
		cur = s.pop()
		fin_order.append(cur)
		visited.add(cur)
		for i in reversed(adj[cur]):
			if i not in visited:
				s.append(i)
	# "*** --------------  ***"
	return fin_order
